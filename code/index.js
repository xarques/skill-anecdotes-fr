...
const LaunchRequestHandler = {
  ...
  handle(handlerInput) {
    // speech (prompt) start
    let speechText =
      'Bienvenue sur la Skill des anecdotes sur les villes françaises.';
    // reprompt
    const repromptText =
      'Pour quelle ville souhaitez-vous connaître une anecdote ?';
    // get session attributes
    const sessionAttributes = handlerInput.attributesManager.getSessionAttributes();
    // append to speech text the number of times, the skill has been launched
    if (sessionAttributes['counter']) {
      speechText +=
        " Vous m'avez invoqué " + sessionAttributes['counter'] + ' fois déjà!';
    }
    // speech (prompt) end
    speechText +=
      " Dites-moi le nom d'une ville et vous apprendrez un fait nouveau sur celle-ci.";
...
const GetNewFactIntentHanlder = {
  ...
  handle(handlerInput) {
    ...
    if (
      ...
      city = 'france';
      speechText +=
        "Je n'ai pas trouvé d'anecdotes sur " +
        slotValues.citySlot.synonym +
        '. Je vous propose une anecdote sur la France : ';
    }
    // find fact
    ...
    if (!handlerInput.requestEnvelope.session['new']) {
      const repromptText =
        'Pour quelle autre ville souhaitez-vous connaître une anecdote ?';
      speechText += ' ' + repromptText;
...
const HelpIntentHandler = {
  ...
  handle(handlerInput) {
    ...
    const speechText =
      'Je vous donne des faits sur les différentes métropoles de France. Il suffit par exemple de me dire "Alexa, donne-moi une anecdote sur Rennes".';
    const repromptText =
      'Pour quelle ville souhaitez-vous connaître une anecdote ?';
    // use responseBuilder to generate the JSON response
    return handlerInput.responseBuilder
      ...
      return new Promise((resolve, reject) => {
        handlerInput.attributesManager
          ...
          .then((s3Attributes) => {
            ...
            const counter = s3Attributes.hasOwnProperty('counter')
              ? s3Attributes.counter
              : 0;
...
const facts = {
  ...
  paris: [
    ...
    'L’un des cafés les plus vieux de Paris est Le Procope qui existe depuis 1686 !',
